# Goal: Task 5 — LLM report.md generator

**Wave:** 2 (parallel with tasks: 2, 3, 4, 6)
**Depends on:** Task 1 (scaffold); `Annotation` type + `makeAnnotation` fixture from Task 2 (wave-2 parallel note below)
**Plan section:** `docs/superpowers/plans/2026-07-20-browser-markup-plan.md` → "### Task 5: LLM report.md generator" — implement those steps in order, checking off the `- [ ]` boxes as you complete them.
...
- Consumes: `Annotation` (Task 2), `makeAnnotation` fixture (Task 2). Wave-2 parallel note: this is dashboard-side code — importing `types.ts` is safe (type-only import, erased; and report-md is imported only by the dashboard entry, never by the content entry).

## Interfaces you produce (later tasks depend on these names — do not rename or change signatures)
- `generateReportMd(annotations: Annotation[]): string`
