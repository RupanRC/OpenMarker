# Goal: Task 9 — Dashboard — session list, edit, delete, reorder

**Wave:** 3 (parallel with tasks: 7)
**Depends on:** Task 2 (`Annotation`), Task 4 (annotation store + screenshot store)
**Plan section:** `docs/superpowers/plans/2026-07-20-browser-markup-plan.md` → "### Task 9: Dashboard — session list, edit, delete, reorder" — implement those steps in order, checking off the `- [ ]` boxes as you complete them.
...
- Consumes: `getAllAnnotations`, `saveAnnotation`, `deleteAnnotation`, `reorderAnnotations` (Task 4);
- `getScreenshot`, `deleteScreenshot` (Task 4);
- `Annotation` (Task 2).

## Interfaces you produce (later tasks depend on these names — do not rename or change signatures)
- Dashboard UI grouped by URL; per-annotation edit (`commentEdited`, updates `updatedAt`), delete (annotation + screenshot blob), reorder (up/down within group);
- export buttons with element ids `bm-export-llm` and `bm-export-html` that Tasks 10/11 wire up (this task renders them disabled).
